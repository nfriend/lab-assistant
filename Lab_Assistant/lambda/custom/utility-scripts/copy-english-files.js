...
// Copy the manifest.publishingInformation.locales.en-US section in skill.json
// to en-CA, en-GB, etc.
const skillJsonPath = path.resolve(__dirname, '../../../skill.json');
const skillJson = require(skillJsonPath);

const enUsLocale = skillJson.manifest.publishingInformation.locales['en-US'];
supportedEnglishCodes.forEach(code => {
  skillJson.manifest.publishingInformation.locales[code] = enUsLocale;
});
